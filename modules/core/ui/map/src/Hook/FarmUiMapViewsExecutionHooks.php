...
class FarmUiMapViewsExecutionHooks
{
    ...
    public function viewsPreView(\Drupal\views\ViewExecutable $view, $display_id, array &$args)
    {
        ...
        if ($view->id() == 'farm_asset_geojson' && in_array($display_id, [
            'full',
            'centroid',
        ])) {
            ...
            $farm_asset_view = \Drupal\views\Entity\View::load('farm_asset');
    ...
    public function viewsPreRender(\Drupal\views\ViewExecutable $view)
    {
        ...
        if ($view->id() == 'farm_asset' && in_array($view->current_display, [
            'page',
            'page_type',
        ])) {
            ...
            foreach ($filtered_bundles as $bundle => $bundle_info) {
                ...
                $type = \Drupal\asset\Entity\AssetType::load($bundle);
                // Load the map layer style.
                /** @var \Drupal\farm_map\Entity\LayerStyleInterface|null $layer_style */
                if ($layer_style = \Drupal::service('farm_map.layer_style_loader')->load([
                    'asset_type' => $bundle,
                ])) {
